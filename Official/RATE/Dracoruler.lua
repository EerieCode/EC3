...
function c7519.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DESTROY+CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCountLimit(1,7519)
	e1:SetTarget(c7519.sptg)
	e1:SetOperation(c7519.spop)
	c:RegisterEffect(e1)
	--spsummon2
	...
	e2:SetCountLimit(1,7519+1)
	e2:SetCondition(c7519.spcon2)
	e2:SetTarget(c7519.sptg2)
	e2:SetOperation(c7519.spop2)
	c:RegisterEffect(e2)
end
function c7519.desfilter(c)
	return c:IsType(TYPE_MONSTER) and ((c:IsLocation(LOCATION_MZONE) and c:IsFaceup()) or c:IsLocation(LOCATION_HAND))
end
function c7519.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local loc=LOCATION_MZONE+LOCATION_HAND
	if ft<0 then loc=LOCATION_MZONE end
	local g=Duel.GetMatchingGroup(c7519.desfilter,tp,loc,0,c)
...
function c7519.rmfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToRemove()
end
function c7519.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local loc=LOCATION_MZONE+LOCATION_HAND
	if ft<0 then loc=LOCATION_MZONE end
	...
	if Duel.Destroy(g1,REASON_EFFECT)==2 then
		...
		if rm and rg:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(7519,0)) then
			local count=3
			local stop=false
			...
			while count>0 and not stop do
				...
				count=count-1
				if rg:GetCount()==0 or count==0 or not Duel.SelectYesNo(tp,aux.Stringid(7519,1)) then
					stop=true
				end
			end
...
function c7519.spcon2(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_EFFECT)
end
function c7519.thfilter(c,e,tp)
	return not c:IsAttribute(ATTRIBUTE_EARTH) and c:IsRace(RACE_WYRM) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c7519.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c7519.thfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c7519.spop2(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c7519.thfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
