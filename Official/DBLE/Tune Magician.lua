...
function c7652.spfil(c,e,tp)
	return c:IsType(TYPE_PENDULUM) and c:IsSetCard(SET_MAGICIAN) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
function c7652.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SpecialSummon(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
...
--Synchro Limit
c7652.tuner_filter=function(c) return c:IsType(TYPE_PENDULUM) and c:IsSetCard(SET_MAGICIAN) end
function c7652.synfilter(c,syncard,tuner,f)
	return c:IsFaceup() and c:IsNotTuner() and c:IsCanBeSynchroMaterial(syncard,tuner) and c:IsSetCard(SET_MAGICIAN) and (f==nil or f(c))
end
