...
function c7485.cfil(c)
	return c:IsSetCard(SET_SUB) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
end
...
function c7485.thfil(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(SET_SUB) and c:IsAbleToHand()
end
...
function c7485.posfil(c)
	return c:IsFaceup() and c:IsSetCard(SET_SUB) and c:IsCanTurnSet()
end
