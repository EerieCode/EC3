...
function c7357.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return c7357[tp]>0 and Duel.IsPlayerCanDraw(tp,c7357[tp]) end
...
function c7357.addcount(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:IsLocation(LOCATION_MZONE) and tc:IsSummonType(SUMMON_TYPE_SYNCHRO) then
