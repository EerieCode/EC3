...
function c7251.desfil(c,tp)
  return c:IsSetCard(SET_ODDEYES) and c:IsType(TYPE_PENDULUM) and c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:GetPreviousControler()==tp and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
end
function c7251.pncon(e,tp,eg,ep,ev,re,r,rp)
  return eg:isExists(c7251.desfil,1,nil,tp)
end
function c7251.pncfil(c)
  return (c:GetSequence()==6 or c:GetSequence()==7) and c:IsDestructable()
end
function c7251.pnfil(c)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c:IsSetCard(SET_ODDEYES) and not c:IsCode(7251) and not c:IsForbidden()
end
...
function c7251.incon(e,tp,eg,ep,ev,re,r,rp)
  ...
  return (c1 and c1:IsSetCard(SET_ODDEYES)) or (c2 and c2:IsSetCard(SET_ODDEYES))
end
function c7251.infil(c)
  return c:IsFaceup() and c:IsSetCard(SET_ODDEYES)
end
