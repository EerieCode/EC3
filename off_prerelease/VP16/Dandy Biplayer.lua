...
function c6138.thfil(c)
	return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c6138.aafil(c) and (c:GetLevel()==1 or c:GetLevel()==8) c:IsAbleToHand()
end
...
function c6138.spfil(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and c6138.aafil(c) and (c:GetLevel()==1 or c:GetLevel()==8) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c6138.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and Duel.IsExistingMatchingCard(c6138.spfil,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c6138.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c6138.spfil,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
